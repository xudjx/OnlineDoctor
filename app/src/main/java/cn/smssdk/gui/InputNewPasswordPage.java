...
/** 短信注册页面*/
public class InputNewPasswordPage extends FakeActivity implements OnClickListener {
    ...
    private Dialog pd;
    //手机区号、手机号、验证码
    private String code, phone, verificationCode;
            ...
            runOnUIThread(new Runnable() {
                public void run() {
                    ...
                    if (result == SMSSDK.RESULT_COMPLETE) {
                        // 加入自己的跳转逻辑
                        Toast.makeText(activity, "加入自己的跳转逻辑", Toast.LENGTH_SHORT).show();
                    } else {
    ...
    public void setPhone(String code, String phone, String verificationCode){
        this.code = code;
        this.phone = phone;
    ...
    public void onCreate() {
        int resId = getLayoutRes(activity, "input_password");
        if (resId > 0) {
            ...
            resId = getStringRes(activity, "smssdk_input_new_password");
            if (resId > 0) {
                tv.setText(resId);
            }
            ...
            etPassword_again.addTextChangedListener(new TextWatcher() {
    ...
    public void onClick(View v) {
        ...
        int id_iv_clear_password = getIdRes(activity, "iv_clear_password");
        int id_iv_clear_password_again = getIdRes(activity, "iv_clear_password_again");
        if (id == id_ll_back) {
            finish();
        ...
        } else if (id == id_iv_clear_password){
            // 清楚密码输入框
            etPassword.getText().clear();
        }else if (id == id_iv_clear_password_again){
            // 清楚密码输入框
...
//        page.setPhone(phone, code, formatedPhone, password);
